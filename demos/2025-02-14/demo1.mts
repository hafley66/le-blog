...
import $ from "jquery"
import { drawTick, mergeByKeyScan } from "~/lib/lib.dual.ts"
import {
  fromEventDelegate,
  mouseMove$,
} from "~/lib/lib.dom.ts"
...
const velocityX$ = fromEventDelegate(
  "#velocityX",
  "input",
).pipe(map(() => +($("#velocityX").val() as string) || 2))

const velocityY$ = fromEventDelegate(
  "#velocityY",
  "input",
).pipe(map(() => +($("#velocityY").val() as string) || 2))

const inputChange$ = fromEventDelegate(
  "#count",
  "input",
).pipe(
