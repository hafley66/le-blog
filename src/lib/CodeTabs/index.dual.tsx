...
type ResultFns<T> = {
  [K in keyof T]: T[K] extends (k: any) => infer R
    ? () => R
    : never
} & {
  _: {
    [K in keyof T]: T[K] extends (k: infer KArg) => infer R
      ? R
      : never
  }
}
...
const hmm = S.x("add-form").declare({
  firstname: k => k + "-lol",
  lastname: k => k.path + "-lol",
  age: k => k,
})

console.log(hmm.firstname()) // "firstname-lol"
console.log(hmm.lastname()) // "src/file/a/add-form/lastname-lol"
console.log(hmm.age()) // { path, dirname, filename }
console.log(hmm._) // Raw values
