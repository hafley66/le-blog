...
function lightDismissOpenDialogs(event) {
  console.log("lightDismissOpenDialogs", { event })
...
function preventEscapeCloseForNoneDialogs(event) {
  console.log("preventEscapeCloseForNoneDialogs", event)
  if (!event.isTrusted) {
    console.log("preventEscapeCloseForNoneDialogs A", event)
    return
  }
  if (event.key !== "Escape") {
    console.log("preventEscapeCloseForNoneDialogs B", event)
    return
  }
  // Composed path allows us to find the target within shadowroots
  const target = event.composedPath()[0]
  if (!target) {
    console.log("preventEscapeCloseForNoneDialogs C", event)
    return
  }
  const document = target.ownerDocument
  if (dialogList.has(document)) {
    console.log("preventEscapeCloseForNoneDialogs D", event)
    const topDialog = Array.from(
      dialogList.get(document),
    ).at(-1)
    if (
      topDialog.closedBy === "none" &&
      topDialog.matches(":modal")
    ) {
      console.log(
        "preventEscapeCloseForNoneDialogs E",
        event,
      )
...
export function apply() {
  ...
  Object.defineProperty(
    ...
    "closedBy",
    {
      ...
      set(newValue) {
        if (
          ["none", "any", "closerequest"].includes(newValue)
        )
  ...
  const observer = new MutationObserver(mutations => {
    for (const mutation of mutations) {
      ...
      if (
        ...
        if (
          mutation.oldValue !==
          mutation.target.getAttribute("closedby")
        )
  ...
  document.addEventListener(
    "keydown",
    preventEscapeCloseForNoneDialogs,
  )
}
